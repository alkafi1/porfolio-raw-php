...
$check_status = "SELECT * FROM skills WHERE id='$id'";
$check_status_res = mysqli_query($db_con, $check_status);
$after_assoc = mysqli_fetch_assoc($check_status_res);

if($after_assoc['status'] == 1){
    $active_count = "SELECT COUNT(*) as active_total FROM skills WHERE status=1";
    ...
    if($active_count_assoc['active_total'] == 2){
        ...
        $active_query = "UPDATE skills SET status=0 WHERE id='$id'";
    ...
    if($active_count_assoc['active_total'] == 4){
        ...
        $active_query = "UPDATE skills SET status=1 WHERE id='$id'";
        $active_query_res = mysqli_query($db_con, $active_query);
        header('location:view_skill.php');
    }
