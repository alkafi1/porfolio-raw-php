            ...
            if($_FILES['profile_photo']['name'] != ''){
                ...
                if(in_array($extention, $allowed_ext)){
                    if($profile_photo['size'] <= 100000000000000){
                        ...
                        $_SESSION['profile_update_success'] = "User update Successfull.";
                        header('location:edit_profile.php');

                    }
                    else{
                        $_SESSION['invalid_size'] = "Profile image max size 1 Mb!";
                        header('location:edit_profile.php');
                    }
                }
                else{
                ...
                $update = "UPDATE users SET name='$name', email='$email' WHERE id='$id' ";
                $update_res = mysqli_query($db_con, $update);
                $_SESSION['profile_update_success'] = "Profile update Successfull.";
                header('location:edit_profile.php');
            }
